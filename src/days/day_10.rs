...
/// V. (V can be negative.) noop takes one cycle to complete. It has no other effect.
...
/// cycle, X is still 1. During the third cycle, X is still 1. After the third cycle, the addx 3
/// instruction finishes execution, setting X to 4. At the start of the fourth cycle, the addx -5
/// instruction begins execution. During the fourth cycle, X is still 4. During the fifth cycle, X
/// is still 4. After the fifth cycle, the addx -5 instruction finishes execution, setting X to -1.
...
/// 1 - 8 + 13 + 4 = 21.) During the 60th cycle, register X has the value 19, so the signal strength
/// is 60 * 19 = 1140. During the 100th cycle, register X has the value 18, so the signal strength
/// is 100 * 18 = 1800. During the 140th cycle, register X has the value 21, so the signal strength
/// is 140 * 21 = 2940. During the 180th cycle, register X has the value 16, so the signal strength
/// is 180 * 16 = 2880. During the 220th cycle, register X has the value 18, so the signal strength
/// is 220 * 18 = 3960. The sum of these signal strengths is 13140.
///
...
pub fn day_10() {
    ...
    let mut twenties_strength = 0;

    let mut crt: Vec<char> = vec!['.'; 40 * 6];

    for instruction in instructions {
        let (cycles, add_v): (i32, Option<i32>) = match instruction {
            "noop" => (1, None),
            _ => (
                2,
                Some(instruction.split_once(' ').unwrap().1.parse().unwrap()),
            ),
        };

        for _ in 0..cycles {
            let curr_pixel_x: i32 = (current_cycle % 40).try_into().unwrap();
            let curr_pixel = &mut crt[current_cycle];

            let sprite_center = reg_x;
            if sprite_center - 1 <= curr_pixel_x && curr_pixel_x <= sprite_center + 1 {
                *curr_pixel = '#';
            }

            current_cycle += 1;
            if current_cycle <= 220 && (current_cycle + 20) % 40 == 0 {
                let current_cycle: i32 = current_cycle.try_into().unwrap();
                twenties_strength += current_cycle * reg_x;
            }
        }

        if let Some(add_v) = add_v {
            reg_x += add_v;
        }
    }

    let newline = ['\n'];
    let crt_string: String = crt
        .chunks(40)
        .flat_map(|x| x.iter().chain(newline.iter()))
        .collect();

    println!("Part 1: {twenties_strength}");
    println!("Part 2:\n{crt_string}");
}
