...
/// while the second Elf was assigned sections 6-8 (sections 6, 7, 8). The Elves in the second pair
/// were each assigned two sections. The Elves in the third pair were each assigned three sections:
/// one got sections 5, 6, and 7, while the other also got 7, plus 8 and 9. This example list uses
/// single-digit section IDs to make it easier to draw; your actual list might contain larger
...
pub fn day_04() {
    ...
    for line in data_as_lines {
        ...
        if let Some((first_elf, second_elf)) = trimmed_line.split_once(',') {
            ...
            let min_second_elf: u64 = min_second_elf.parse().unwrap();
            let max_second_elf: u64 = max_second_elf.parse().unwrap();

            // PartialCmp exists for &str, kind of annoying
            if (min_first_elf <= min_second_elf && max_first_elf >= max_second_elf)
                || (min_second_elf <= min_first_elf && max_second_elf >= max_first_elf)
            {
                count_part_1 += 1;
            }
