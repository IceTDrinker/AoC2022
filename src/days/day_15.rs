...
// where the distress beacon is by working out where it isn't. For now, keep things simple by
...
// the row where y=10, you'd like to count the number of positions a beacon cannot possibly exist.
...
pub fn day_15() {
    ...
    impl Sensor {
        pub fn distance_to_closest_beacon(&self) -> u32 {
    ...
    let (min_x, max_x) = sensors.iter().fold((i32::MAX, i32::MIN), |acc, sensor| {
        let (mut min_x, mut max_x) = acc;

        min_x = std::cmp::min(
            sensor.pos.x - sensor.distance_to_closest_beacon() as i32,
            min_x,
        );
        max_x = std::cmp::max(
            sensor.pos.x + sensor.distance_to_closest_beacon() as i32,
    ...
    no_beacon_locations -= beacons_at_y.len() as i32;
